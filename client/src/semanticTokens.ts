...
export class SemanticTokensProvider implements vscode.DocumentSemanticTokensProvider
{
	register()
	{
		...
		vscode.languages.registerDocumentSemanticTokensProvider({ language: 'merlin6502' }, this, legend);
	}
	provideDocumentSemanticTokens(document:vscode.TextDocument): vscode.ProviderResult<vscode.SemanticTokens>
	{
		try {
			return client.sendRequest(vsclnt.ExecuteCommandRequest.type, {
				command: 'merlin6502.semantic.tokens',
				arguments: [document.getText(),document.uri.toString()]
			});
		} catch (error) {
