...
export class TSDiagnosticProvider extends lxbase.LangExtBase
{
	...
	value_range(diag: Array<vsserv.Diagnostic>,node: Parser.SyntaxNode,low:number,high:number)
	{
		if (node.type!="integer")
			return;
		const rng = lxbase.node_to_range(node,this.row,this.col);
	...
	visit_verify(curs: Parser.TreeCursor): lxbase.WalkerChoice
	{
		...
		else if (this.config.case.caseSensitive && (curs.currentNode().type.substring(0,3)=='op_' || curs.currentNode().type.substring(0,5)=='psop_'))
		{
