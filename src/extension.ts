...
export function activate(context: vscode.ExtensionContext)
{
	lxbase.TreeSitterInit().then( TSInitResult =>
	{
		...
		if (vscode.window.activeTextEditor?.document.languageId=='merlin6502')
		{
			diagnostics.update(vscode.window.activeTextEditor.document, collection);
			typeIndicator.text = diagnostics.get_interpretation(vscode.window.activeTextEditor.document);
